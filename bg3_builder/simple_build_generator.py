...
from .build_fetcher import create_korean_blog_post
from .simple_perplexity_extractor import process_simple_perplexity_images
from .markdown_inserter import remove_existing_summary_cards
from .utils import logger

def create_build_simple_system(build_name, perplexity_file_path):
    """
    간단한 Perplexity 시스템으로 빌드 가이드 생성
    ...
    """
    logger.info(f"🚀 간단한 Perplexity 시스템으로 {build_name} 빌드 가이드 생성 시작")
    ...
    try:
        ...
        # 4. 간단한 Perplexity 이미지 시스템 적용
        logger.info("4️⃣ 간단한 Perplexity 이미지 시스템으로 이미지 처리...")
        final_content = process_simple_perplexity_images(perplexity_raw_response, clean_korean_post)
        
        # 5. 결과 저장
        logger.info("5️⃣ 빌드 데이터 저장...")
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"simple_build_data_{build_name.replace(' ', '_')}_{timestamp}.json"
            ...
            build_data = {
                ...
                "statistics": {
                    ...
                    "system_version": "simple_perplexity"
                },
                "metadata": {
                    "source": "perplexity_file", 
                    "translator": "grok_api",
                    "image_system": "simple_perplexity_extraction",
                    "version": "1.0_simple_system"
                }
            }
            
            with open(filename, 'w', encoding='utf-8') as f:
                json.dump(build_data, f, ensure_ascii=False, indent=2)
            logger.info(f"빌드 데이터가 {filename}에 저장되었습니다.")
        ...
        logger.info("🎯 간단한 Perplexity 시스템으로 빌드 가이드 생성 완료!")
