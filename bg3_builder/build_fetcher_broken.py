...
def create_korean_blog_post(build_info, build_name):    """영어 빌드 정보를 한국어 블로그 포스트로 변환"""    logger.info(f"Grok API를 통해 {build_name} 한국어 블로그 포스트 생성 중...")        # 빌드명을 프롬프트에 삽입    translation_prompt = TRANSLATION_PROMPT_TEMPLATE.format(        build_name=build_name,        build_info=build_info    )        # 먼저 Grok API를 시도    logger.info("Grok API로 한국어 번역 시도...")    korean_blog_post = get_grok_response(translation_prompt)        # Grok API 실패 시 Perplexity API로 대체    if not korean_blog_post:        logger.warning("Grok API 실패, Perplexity API로 대체...")        korean_blog_post = get_perplexity_response(translation_prompt)        if korean_blog_post:        logger.info("한국어 블로그 포스트 생성 완료!")        return korean_blog_post    else:        logger.error("한국어 블로그 포스트 생성 실패.")        return None 